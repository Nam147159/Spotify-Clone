    ...
    this.mainPanelService.getPopularAlbums().subscribe({
      ...
      next: (response: {
        ...
        if (response.success && Array.isArray(response.data)) {
          ...
          this.popularAlbums = this.popularAlbums.filter(
            (album) =>
              album &&
              album.name &&
              album.artists &&
    ...
    this.mainPanelService.getRecommendedPlaylists().subscribe({
      next: (response: {
        ...
        if (response.success && Array.isArray(response.data)) {
          ...
          this.recommendedPlaylists = this.recommendedPlaylists.filter(
            (playlist) =>
              playlist &&
              playlist.name &&
    ...
    this.mainPanelService.getTop100Playlists().subscribe({
      next: (response: {
        ...
        if (response.success && Array.isArray(response.data)) {
          ...
          //Filter data that has name, description, and images
          this.top100Playlists = this.top100Playlists.filter(
            (playlist) =>
              playlist &&
              playlist.name &&
              playlist.description &&
